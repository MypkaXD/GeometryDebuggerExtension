...
cmake -S "%INSTALL_DIR%/%INSTALL_DIR%" -B "%INSTALL_DIR%/%INSTALL_DIR%/build" -G "Visual Studio 17 2022" -A x64 -DBUILD_SHARED_LIBS=OFF -DCMAKE_INSTALL_PREFIX="%INSTALL_DIR%/%INSTALL_DIR%/install" -DGLFW_BUILD_DOCS=OFF -DGLFW_BUILD_EXAMPLES=OFF -DGLFW_BUILD_TESTS=OFF -DGLFW_BUILD_WIN32=ON -DGLFW_INSTALL=ON -DGLFW_USE_HYBRID_HPG=OFF -USE_MSVC_RUNTIME_LIBRARY_DLL=OFF -DCMAKE_BUILD_TYPE=Debug 

echo [6/6] Building
cmake --build "%INSTALL_DIR%/%INSTALL_DIR%/build" --config Debug --target install

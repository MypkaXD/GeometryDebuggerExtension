...
std::vector<std::vector<std::pair<float, float>>> get_cut_of_figure(BoundingBox box, std::vector<Edge> edges, std::ofstream& file) {
	...
	if (edges.size() != 1 && edges.size() != 2)
		...
		std::tuple<float, float, float> equation_of_left = get_equation_of_line(Point(box.m_x_min, box.m_y_min, 0),
			Point(box.m_x_min, box.m_y_min + box.m_height, 0));
		std::tuple<float, float, float> equation_of_right = get_equation_of_line(Point(box.m_x_min + box.m_width,
			box.m_y_min, 0), Point(box.m_x_min + box.m_width, box.m_y_min + box.m_height, 0));
		std::tuple<float, float, float> equation_of_up = get_equation_of_line(Point(box.m_x_min,
			box.m_y_min + box.m_height, 0), Point(box.m_x_min + box.m_width, box.m_y_min + box.m_height, 0));
		std::tuple<float, float, float> equation_of_bottom = get_equation_of_line(Point(box.m_x_min,
			box.m_y_min, 0), Point(box.m_x_min + box.m_width, box.m_y_min, 0));
		...
		std::vector<std::pair<float, float>> lines_of_box = { std::make_pair(box.m_x_min, box.m_y_min), std::make_pair(box.m_x_min, box.m_y_min + box.m_height),
			std::make_pair(box.m_x_min + box.m_width, box.m_y_min + box.m_height), std::make_pair(box.m_x_min + box.m_width, box.m_y_min) };
		...
		dump_points_on_box(points_on_box, file, "points_on_box");
		...
		dump_points_on_box(result_points, file, "result");

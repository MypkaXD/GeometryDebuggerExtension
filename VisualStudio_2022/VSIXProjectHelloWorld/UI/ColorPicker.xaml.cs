...
namespace VSIXProjectHelloWorld
{
    ...
    public partial class ColorPicker : UserControl
    {
        ...
        {
            if (RedValue != null && int.TryParse(RedValue.Text, out int red) && red >= 0 && red <= 255)
                RedSlider.Value = red;
            else if (RedSlider != null)
                RedSlider.Value = 0;

            if (GreenValue != null && int.TryParse(GreenValue.Text, out int green) && green >= 0 && green <= 255)
                GreenSlider.Value = green;
            else if (GreenSlider != null)
                GreenSlider.Value = 0;

            if (BlueValue != null && int.TryParse(BlueValue.Text, out int blue) && blue >= 0 && blue <= 255)
